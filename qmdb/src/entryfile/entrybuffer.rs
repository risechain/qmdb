...
impl EntryBuffer {
        ...
        F: FnMut(EntryBz),
    {
        ...
        access(entry_bz);
        (false /*inDisk*/, true /*HaveAccessed*/, curr_buf)
    }
}
...
mod test_entry_buffer {
    ...
    use crate::{
        ...
        entryfile::{entry::entry_to_bytes, Entry},
    };

    #[cfg(feature = "tee_cipher")]
    #[test]
    #[should_panic(expected = "Entry too large 100072 vs 65536")]
    ...
    #[test]
    #[should_panic(expected = "Entry too large 100056 vs 65536")]
    fn test_append_panic() {
        let (mut writer, _reader) = new(0, 3 * BIG_BUF_SIZE);
        writer.append(
            &Entry {
    ...
    fn test_threshold_size() {
        ...
        for i in 0..2 {
            writer.get_entry_bz_at(start + i as i64 * total_size as i64, |entry_bz| {
                assert_eq!(entry_bytes, entry_bz.bz);
            });
        }
